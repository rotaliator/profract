...
cdef int mandel_for(float re, float im, double max_dist=2**6,int max_iter=255):
    z_re, z_im = re, im
...
def mandel_classic(re1, im1, re2, im2, width, height):
    ...
    """
    a = array.array('B', repeat(255, width*height))
...
def mandel_cython(re1, im1, re2, im2, width, height):
    """
    Calculates iterations of mandelbrot set into array of floats
    pure Python reference implementation
    re1, im1 - upper left corner
    ...
    return array.array('B', map(f, points))
